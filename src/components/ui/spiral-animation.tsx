...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    this.z =
      this.z * (1 - 0.3 * this.spiralLocation) +
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (displacementProgress < 0.3) {
      ...
      screenX =
        controller.lerp(baseX, targetX, midProgress) + -this.dy * 0.4 * curveStrength * midProgress;
      screenY =
        controller.lerp(baseY, targetY, midProgress) + this.dx * 0.4 * curveStrength * midProgress;
    } else {
